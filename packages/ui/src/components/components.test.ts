...
import { DmButton, DmSplitPane, DmStatus, DmTextField } from '../index.js';
...
describe('UI component rendering contracts', () => {
  it('renders a loading button as disabled and exposes only the loading label', async () => {
    ...
    expect(html).toContain('aria-hidden="true">保存模板</span>');
  ...
  it('keeps one split pane visible if a controlled parent collapses both', async () => {
    ...
    expect(html).toContain('aria-label="原件">原始合同</section>');
    expect(html).toContain('aria-label="模板" hidden>可编辑模板</section>');
  });
});
