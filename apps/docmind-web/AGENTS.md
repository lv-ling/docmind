...
- 未经明确同意，不新增生产依赖，不引入 Tailwind CSS、UnoCSS 或其他原子 CSS 框架。
...
├── layouts/                     页面布局与应用外壳
├── router/
│   └── modules/                 每个业务领域一个路由模块
├── stores/                      跨页面、跨路由的全局状态
├── styles/                      基础样式、设计令牌入口和原子类
├── ui/                          无业务语义的设计系统基础组件
├── utils/                       与业务无关的纯工具
└── views/
    └── <domain>/
...
- 每个业务领域在 `router/modules` 中使用独立文件，例如 `source.ts`、`schema.ts`、`extraction.ts`、`template.ts`。
...
- 测试使用 `*.test.ts` 或 `*.test.tsx`，与被测模块就近放置。
...
- 新页面和新组件应尽可能通过项目原子类组合布局、间距、尺寸、排版、颜色和常见状态。
- 原子类是一条职责单一、基于设计令牌、可跨页面复用的 CSS 类，不等同于随意创建大量一次性短类名。
- 当前项目没有原子 CSS 框架。未经明确批准，不得引入 Tailwind CSS、UnoCSS 或改变构建链；优先建设项目自有的轻量原子类层。
- 不得使用尚未定义的原子类。新增原子能力时，应在统一 utility 样式文件中定义，并使用现有设计令牌和尺寸刻度。

### 8.2 原子类命名和范围

- 项目自有原子类统一使用 `u-` 前缀，避免与业务类和 `dm-` 设计系统类混淆。
- 原子类使用清晰、稳定的职责名称，例如：
...
- 间距、字号、圆角、颜色和阴影必须映射到统一令牌或刻度，禁止为单个页面创建 `u-mt-13px`、`u-color-a17f3c` 等一次性类。
...
- 全局样式只允许放重置、基础元素、设计令牌、原子类和真正的全应用规则。
- 页面专属选择器不得继续追加到巨型 `src/styles.css` 末尾。
