...
import { DmButton } from '@docmind/ui';
...
const load = async (): Promise<void> => {
  ...
  try {
    ...
    error.value = caught instanceof Error ? caught.message : '抽取配置加载失败';
  } finally {
    loading.value = false;
  }
};
...
<template>
  <section class="page-stack extraction-create-page">
    ...
    <InlineNotice v-if="error" tone="danger" title="暂时无法发起任务" :detail="error" />
    <form v-if="!loading" class="extraction-order" @submit.prevent="submit">
      ...
      <footer>
        ...
        <DmButton type="submit" :disabled="schemaVersionId === ''" :loading="submitting"
