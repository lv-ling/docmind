...
> 导航：[仓库首页](../../README.md) / [应用层](../README.md) / DocMind Server

`docmind-server` 是 Java 17 + Spring Boot 3.5 业务后端，是身份、工作区、权限、业务数据和敏感映射的访问边界。当前实现覆盖身份与工作区、原件、Schema、敏感规则、抽取、模板、异步任务、审计和基础设施适配；实例、协作和校对等后续领域以 OpenAPI 目标契约为准。

## 代码结构

```text
apps/docmind-server/
├── contracts/              Server 提供的 OpenAPI、事件和业务 JSON Schema
├── .mvn/wrapper/           Maven Wrapper 配置
├── mvnw / mvnw.cmd         固定 Maven 3.9.12 的跨平台入口
...
前置条件：Java 17 和 Docker。Maven 由本应用的 Wrapper 固定为 3.9.12，无需本机另行安装；首次执行 Wrapper 会下载该版本。从仓库根目录启动依赖，再运行 Server：

```bash
# 启动并检查 PostgreSQL、Redis、RabbitMQ 和 MinIO
./scripts/dev/infra.sh start

# 通过 Maven Wrapper 启动 Spring Boot
...
响应和结构化日志均携带 UUID 格式的 `X-Request-ID`；合法的调用方请求 ID 会原样透传，非法值会被替换。异常响应遵循本应用 [`contracts`](contracts/README.md) 中定义的 snake_case 错误结构，未知异常不会暴露原始消息或文档内容。
...
# 运行完整 Maven 验证并生成可执行 JAR
./mvnw verify

# 从仓库根目录执行同一 Server 门禁
./scripts/dev/server.sh verify

# 在基础设施和服务均运行时执行跨服务黑盒验收
./scripts/dev/e2e.sh mvp
```

`test` profile 使用 H2 隔离数据库并关闭 Redis、RabbitMQ、MinIO 健康依赖，验证请求关联、错误契约、存活探针、JWT、幂等、对象级权限、审计、文件真实性、staging/immutable 隔离、抽取任务边界、发布确认、重复投递、重试、死信决策、AI 熔断、加密信封以及含敏感多候选的完整抽取持久化链路。需要验证真实依赖、Flyway PostgreSQL 迁移和 RabbitMQ Broker 路由时，先执行 `./scripts/dev/infra.sh start`，再分别启动 Document AI 与 Server，最后执行 `./scripts/dev/e2e.sh mvp`。
...
相关入口：[Server 契约](contracts/README.md)、[DocMind Document AI](../docmind-document-ai/README.md)、[本地基础设施](../../deploy/compose/README.md)、[跨服务验收](../../tests/e2e/README.md)。
